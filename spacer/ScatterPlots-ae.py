...
def plotScatter(f1, f2, a, out):
		...
		f2Name,ext=os.path.splitext(os.path.basename(f2))
		df1=pandas.read_csv(f1);
		df2=pandas.read_csv(f2);
		combinedDF=pandas.merge(df1,df2,how="left",on="index")
		combineddf=combinedDF[pandas.notnull(combinedDF["Result_x"])]
		combineddf=combinedDF[pandas.notnull(combinedDF["Result_y"])]
		m=max(combinedDF["execution_time_x"].max(),combinedDF["execution_time_y"].max())
		combinedDF.loc[combinedDF["Result_x"]=="UNKNOWN",["execution_time_x"]]=m
		combinedDF.loc[combinedDF["Result_y"]=="UNKNOWN",["execution_time_y"]]=m
